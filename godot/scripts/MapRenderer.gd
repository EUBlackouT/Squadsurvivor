...
#
# To remove later: delete the MapRenderer scene and set Main.gd `use_rich_map = false`.
...
# Optional richness layer (easy to disable/remove)
...
func _try_set(obj: Object, prop: StringName, value: Variant) -> void:
	# Godot 4 doesn't have has_variable(); use property list for safe optional sets.
	if obj == null:
		return
...
func _setup_ground() -> void:
	...
	# 1×1 white texture so the shader draws a full quad we can scale.
	if _white_tex == null:
	...
	_mat.shader = preload("res://shaders/map_ground_rich.gdshader")
	ground.material = _mat
...
func _setup_fog() -> void:
	...
	# Share the same 1×1 white texture.
	if _white_tex == null:
		var img := Image.create(1, 1, false, Image.FORMAT_RGBA8)
		img.fill(Color.WHITE)
		_white_tex = ImageTexture.create_from_image(img)
...
func _apply_theme(id: String) -> void:
	...
	match id:
		"arcane_ruins":
			_mat.set_shader_parameter("u_base_color", Color(0.07, 0.10, 0.13, 1.0))
			_mat.set_shader_parameter("u_alt_color", Color(0.10, 0.13, 0.18, 1.0))
			_mat.set_shader_parameter("u_accent_color", Color(0.22, 0.35, 0.48, 1.0))
			_mat.set_shader_parameter("u_scale", 0.018)
			_mat.set_shader_parameter("u_detail_scale", 0.10)
			_mat.set_shader_parameter("u_crack_strength", 0.95)
			_mat.set_shader_parameter("u_grit_strength", 0.65)
			_mat.set_shader_parameter("u_vignette", 0.30)
			if _fog_mat != null:
				_fog_mat.set_shader_parameter("u_fog_color", Color(0.55, 0.75, 0.95, 1.0))
		_:
			# graveyard default: slate + mossy accents
			_mat.set_shader_parameter("u_base_color", Color(0.08, 0.10, 0.12, 1.0))
			_mat.set_shader_parameter("u_alt_color", Color(0.12, 0.13, 0.16, 1.0))
			_mat.set_shader_parameter("u_accent_color", Color(0.18, 0.22, 0.18, 1.0))
			_mat.set_shader_parameter("u_scale", 0.020)
			_mat.set_shader_parameter("u_detail_scale", 0.092)
			_mat.set_shader_parameter("u_crack_strength", 1.05)
			_mat.set_shader_parameter("u_grit_strength", 0.85)
			_mat.set_shader_parameter("u_vignette", 0.40)
			if _fog_mat != null:
...
func _update_ground_follow() -> void:
	...
	ground.scale = size # because texture is 1×1
	_mat.set_shader_parameter("u_world_origin", cam_pos - size * 0.5)
	_mat.set_shader_parameter("u_world_size", size)

func _update_fog_follow() -> void:
...
func _spawn_world_props() -> void:
	...
	# Keep it simple: reuse existing structure sheets we already have in the repo.
	...
	while placed < prop_count and tries < prop_count * 20:
		...
		# Depth hint: a tiny y-based z sort.
