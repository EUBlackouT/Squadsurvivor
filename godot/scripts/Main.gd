...
# Map visuals (removable). If disabled, falls back to old checkerboard background.
...
func _make_background() -> void:
	# Preferred: MapRenderer (procedural rich ground + props).
	if use_rich_map and MAP_RENDERER_SCENE != null:
		var mr := MAP_RENDERER_SCENE.instantiate()
		mr.name = "MapRenderer"
		add_child(mr)
		# Best-effort wiring (keeps MapRenderer removable/optional).
	...
	# Fallback: old checkerboard background.
