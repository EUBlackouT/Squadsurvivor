...
# Restores missing PNG sources from Godot .import metadata by decoding the
# cached .ctex files. This is useful when only *.png.import files remain.
...
func _initialize() -> void:
	...
	for import_path in import_files:
		...
		var save_err := img.save_png(source_file)
		if save_err == OK:
			restored += 1
		else:
			skipped += 1
